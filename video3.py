...
def prep_data(images):
    count = len(images)
    data = np.ndarray((count, 1, 64, 64), dtype=np.uint8)
...
if __name__ == '__main__':
    ...
    while(True):
        ...
        img_path = "output.png"
        data = np.ndarray((1, 1, 64, 64), dtype=np.uint8)
        img = cv2.imread(img_path, cv2.IMREAD_GRAYSCALE)
        data = img
        x = data
        x = x.reshape(x.shape[0], 64, 64, 1).astype('float32')
        x = x / 255
        preds = model.predict_classes(test, verbose=2)
        
        print('Label:', preds)
        
        cv2.putText(frame, "Label: {}".format(preds), (10, 30),cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
